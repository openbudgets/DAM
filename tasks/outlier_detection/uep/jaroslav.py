...
datasource_id = upload_data_set(CSV_FILE, CSV_SEPARATOR)
...
miner_id = create_miner(datasource_id, "TEST MINER", API_URL, API_KEY)


#3 preprocess data fields to attributes
def preprocess_data_fields_to_attributes(dataSourceId, apiURL, apiKey):
    headers = {'Content-Type': 'application/json', "Accept": "application/json"}
...
attributes_columns_map = preprocess_data_fields_to_attributes(datasource_id, API_URL, API_KEY)


#4 prepare antecedent pattern
def prepare_antecedent_pattern(antecedentColumns):
    antecedent = []
...
def define_data_mining_task(apiUrl, apiKey, taskName, minerId, minerName, antecedentColumns, consequentColumns, attributesColumnsMap):
    headers = {'Content-Type': 'application/json', "Accept": "application/json"}
    antecedent = prepare_antecedent_pattern(antecedentColumns)
...
task_id = define_data_mining_task(API_URL, API_KEY, "simple", miner_id, "Test Miner", ANTECEDENT_COLUMNS,
                                  CONSEQUENT_COLUMNS, attributes_columns_map)
